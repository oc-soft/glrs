        ...
        for ridx in 0..self.get_row_count() {
            for cidx in 0..self.get_col_count() {
                self.set_component(
                    ridx,
                    cidx,
                    self.get_component(ridx, cidx).unwrap() * scale,
                )
                .unwrap();
            }
        }
...
impl Matrix {
    ...
    pub fn multiply_mut(&mut self, mat: &Self) -> bool {
        if self.dim == mat.dim {
            ...
            .unwrap();
            let mat_1 =
                MatrixI::bind_with_col_count_0(mat.component.clone(), mat.dim)
                    .unwrap();
...
impl Matrix {
    /// outer product
    #[allow(clippy::needless_range_loop)]
    pub fn outer_product(u: &[f64], v: &[f64]) -> Result<Matrix, MatrixError> {
...
impl Matrix {
    ...
    pub fn new_frustum(
            ...
            || fn_v != -0.0
        {
            ...
            result
                .set_component(2, 3, (-2.0 * far * near) / fn_v)
                .unwrap();
    ...
    pub fn new_perspective(
        ...
        if aspect != 0.0 && aspect != -0.0 && nf != 0.0 || nf != -0.0 {
            ...
            result
                .set_component(2, 3, (2.0 * z_far * z_near) / nf)
                .unwrap();
            result.set_component(3, 2, -1.0).unwrap();
    ...
    pub fn new_look_at(
        ...
        if f_len != 0.0 && f_len != -0.0 && up_vec_len != 0.0
            || up_vec_len != -0.0
        {
        ...
        if self.dim == 4 {
            ...
            for idx in 0..3 {
                self.set_component(
                    idx,
                    3,
                    self.get_component(idx, 3).unwrap() + trans_vec[idx],
                )
                .unwrap();
            }
            Ok(self)
        ...
        if self.dim > 2 && self.dim < 5 {
            ...
            for idx in 0..3 {
                self.set_component(
                    idx,
                    idx,
                    self.get_component(idx, idx).unwrap() * scale_vec[idx],
                )
                .unwrap();
            }
            Ok(self)
    ...
    /// scale {x, y, z} component
    pub fn scale3(&self, x: f64, y: f64, z: f64) -> Result<Self, MatrixError> {
        if self.dim > 2 && self.dim < 5 {
            let mut result = self.clone();
