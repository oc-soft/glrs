...
#[wasm_bindgen]
pub fn float_vec_retain(vec: *const Vec<OrderedFloat<f64>>) -> usize {
    unsafe {
...
#[wasm_bindgen]
pub fn float_vec_release(vec: *const Vec<OrderedFloat<f64>>) -> usize {
    unsafe {
...
#[wasm_bindgen]
pub fn float_vec_size(vec: *const Vec<OrderedFloat<f64>>) -> usize {
    if !vec.is_null() {
        unsafe { (*vec).len() }
    } else {
        0 as usize
