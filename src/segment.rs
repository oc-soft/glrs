...
impl Segment {
        ...
        if d.len() > 1 && c.len() > 1 && (t[0] - t[1]).abs() > 0.0 {
            ...
            if d_len > 0.0 {
                ...
                if d_0[dim - 1] != 0.0 || d_0[dim - 1] != -0.0 {
                    ...
                    c_0 =
                        geom::plus(&geom::scale(c_scale, &d_0), &c_0).unwrap();
                    t_0[0] = -c_scale;
                    t_0[1] = t_0[0] + t_diff;
                    Ok(Segment {
                        d: d_0,
                    ...
                    if d_0[dim - 2] != 0.0 || d_0[dim - 2] != -0.0 {
                        ...
                        let c_0 =
                            geom::plus(&geom::scale(c_scale, &d_0), &c_0)
                                .unwrap();
                        t_0[0] = -c_scale;
                        t_0[1] = t_0[0] + t_diff;
                        Ok(Segment {
    ...
    /// constructor
    pub fn create_1(p1: &[f64], p2: &[f64]) -> Result<Segment, GeomError> {
...
impl Segment {
    ...
    pub fn cross_point_parameter_2d_0(
        &self,
        other: &Self,
        tolerance: f64,
    ) -> Option<[f64; 2]> {
        ...
        if (1.0 - d_prod).abs() > tolerance {
            let d_mat = Matrix::new_with_source_component_row_order(&[
                -d_1[1], d_1[0], -d_2[1], d_2[0],
            ]);
            let dc_vec = vec![
                d_1[0] * self.c[1] - d_1[1] * self.c[0],
            ...
            match d_mat.inverse() {
                Some(res_mat) => {
                    ...
                    let t_1 =
                        (res_vec[acc_idx] - self.c[acc_idx]) / d_1[acc_idx];
                    ...
                    let t_2 =
                        (res_vec[acc_idx] - other.c[acc_idx]) / d_2[acc_idx];
                    Some([t_1, t_2])
                }
                _ => None,
            }
    ...
    /// calculate cross point parameter as line 2d
    pub fn cross_point_parameter_2d(&self, other: &Self) -> Option<[f64; 2]> {
